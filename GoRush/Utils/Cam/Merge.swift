...
final class Merge {
     ...
     */
    func overlayVideo(frontCamera: Bool, video: AVAsset,
        ...
        let audioTrack = (audioTracks.isEmpty || mute) ? nil : audioTracks[0]
        ...
        var layerInstruction : LayerInstruction
        
        if frontCamera == true {
            
            
            var transformA:CGAffineTransform  = CGAffineTransform(scaleX: -1.0, y: 1.0)
            transformA = transformA.translatedBy(x: -videoTrack.naturalSize.width, y: 0.0)
            transformA = transformA.rotated(by: CGFloat(Double.pi/2))
            transformA = transformA.translatedBy(x: 0.0, y: -videoTrack.naturalSize.width)
            
            

            
            layerInstruction = LayerInstruction(track: composition.videoTrack, transform:  transformA, duration: video.duration)

        }else{
            
            layerInstruction = LayerInstruction(track: composition.videoTrack, transform:  videoTrack.preferredTransform, duration: video.duration)

        }
        let instruction = Instruction(length: video.duration, layerInstructions: [layerInstruction.instruction])
        let size = Size(size: videoTrack.naturalSize)
        ...
        Exporter(asset: composition.asset, outputUrl: fileUrl, composition: videoComposition.composition).map { exporter in
...
///  A wrapper of NextLevelSessionExporter.
fileprivate final class Exporter {

    fileprivate let session: NextLevelSessionExporter

    var progress: ((_ progress: Float) -> Void)?

    
    init?(asset: AVMutableComposition, outputUrl: URL, composition: AVVideoComposition) {
       
        
        
        
        
        let session = NextLevelSessionExporter(withAsset: asset)
...
//            AVVideoProfileLevelKey: AVVideoProfileLevelH264BaselineAutoLevel as String,
    ...
    func render(complete: @escaping (_ url: URL?) -> Void) {
        ...
        DispatchQueue.global(qos: .utility).async(group: group) {
            ...
            do {
                try self.session.export(completionHandler: { (status) in
                    switch status {
                    case .completed:
...
//            }
    ...
    /**
     Polls the NextLevelSessionExporter status every 500 milliseconds.

     - Parameter session: NextLevelSessionExporter
     - Parameter group: DispatchGroup
     */
    private func progress(session: NextLevelSessionExporter, group: DispatchGroup) {
...
extension MergeConfiguration {
    ...
    static var standard: MergeConfiguration {
            ...
            return MergeConfiguration(frameRate: 30, directory: NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true)[0],  placement: Placement.stretchFit)
