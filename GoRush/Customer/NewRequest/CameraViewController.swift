...
class CameraViewController: SwiftyCamViewController, SwiftyCamViewControllerDelegate{
    ...
    convenience init(videoToAcceptJob: Bool, requestWorkerVC : RequestWorkerViewController)
    {
        
        self.init()
        self.defaultCamera = .front

        self.videoToAcceptJob = videoToAcceptJob
    ...
    override func viewDidLoad() {
        ...
        if self.videoToAcceptJob == true {
            
            self.titleVC.text = NSLocalizedString("Accept Job", comment: "")

        }
        
        if self.videoToStartJob == true {
            
            self.titleVC.text = NSLocalizedString("Start Job", comment: "")

        }
        
        if self.videoToFinishJob == true {
            
            self.titleVC.text = NSLocalizedString("Finish Job", comment: "")
        ...
        if isIphoneXFamily() {
            ...
            flipCameraButton = UIButton(frame: CGRect(x:70, y: originYBottomButtonCTA(), width: 50, height: 50))
        ...
        if isIphoneXFamily() {
            ...
            flashButton = UIButton(frame: CGRect(x:Brain.kLargeurIphone - 70 - 50, y: originYBottomButtonCTA(), width: 50, height: 50))
...
//        
    ...
    func showHelloPopup(){
        ...
        let alert = UIAlertController(title: NSLocalizedString("Say Hi to your client", comment: ""), message: NSLocalizedString("Record a short video of yourself to say hello to your new customer", comment: ""), preferredStyle: UIAlertControllerStyle.alert)
    ...
    func showStartJobPopup(){
        ...
        let alert = UIAlertController(title: NSLocalizedString("Confirm start of job", comment: ""), message: NSLocalizedString("Send a video to your client to confirm the start of your job at the request location", comment: ""), preferredStyle: UIAlertControllerStyle.alert)
        if let popoverController = alert.popoverPresentationController {
        popoverController.sourceView = self.view
        popoverController.permittedArrowDirections = []
        popoverController.sourceRect = CGRect(x: self.view.bounds.midX, y: self.view.bounds.midY, width: 0, height: 0)
    ...
    func showFinishJobPopup(){
        ...
        let alert = UIAlertController(title: NSLocalizedString("Confirm end of job", comment: ""), message: NSLocalizedString("Send a video to your client to confirm the end of your job at the request location", comment: ""), preferredStyle: UIAlertControllerStyle.alert)
        if let popoverController = alert.popoverPresentationController {
        popoverController.sourceView = self.view
        popoverController.permittedArrowDirections = []
        popoverController.sourceRect = CGRect(x: self.view.bounds.midX, y: self.view.bounds.midY, width: 0, height: 0)
    ...
    func swiftyCam(_ swiftyCam: SwiftyCamViewController, didTake photo: UIImage) {
        ...
        if self.videoToAcceptJob == true {
            ...
            self.requestWorkerVC!.viewToAcceptJob(photo: photo, video: nil)
            self.navigationController?.popViewController(animated: true)

            
        }else if self.videoToStartJob == true{
            
            self.requestWorkerVC!.viewToStartJob(photo: photo, video: nil)
            self.navigationController?.popViewController(animated: true)
            
        }else if self.videoToFinishJob == true{
            
            self.requestWorkerVC!.viewToFinishJob(photo: photo, video: nil)
            self.navigationController?.popViewController(animated: true)
                 ...
                 if currentCamera == .front {
                     ...
                     let newVC = SendRequestViewController(photoTmp: photo, videoTmp: nil, frontCamera: true, request: self.request)
                  self.navigationController?.pushViewController(newVC, animated: true)
                  
                 }else{
                     
                     let newVC = SendRequestViewController(photoTmp: photo, videoTmp: nil, frontCamera: false, request: self.request)
                     self.navigationController?.pushViewController(newVC, animated: true)
...
//        self.request.setObject(url.absoluteString, forKey: Brain.kRequestUrl)
        ...
        if self.videoToAcceptJob == true {
            ...
            self.requestWorkerVC!.viewToAcceptJob(photo: nil, video: url)
            self.navigationController?.popViewController(animated: true)
            
       }else if self.videoToStartJob == true{
           
           self.requestWorkerVC!.viewToStartJob(photo: nil, video: url)
           self.navigationController?.popViewController(animated: true)
           
       }else if self.videoToFinishJob == true{
           
           self.requestWorkerVC!.viewToFinishJob(photo: nil, video: url)
           self.navigationController?.popViewController(animated: true)
                       ...
                       if currentCamera == .front {
                           ...
                           let newVC = SendRequestViewController(photoTmp: nil, videoTmp: url, frontCamera: true, request: self.request)
                           self.navigationController?.pushViewController(newVC, animated: true)

                       }else{
                      
                           let newVC = SendRequestViewController(photoTmp: nil, videoTmp: url, frontCamera: false, request: self.request)
                           self.navigationController?.pushViewController(newVC, animated: true)
